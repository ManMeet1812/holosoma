...
from holosoma.config_types.algo import FastTD3Config
...
class FastTD3Agent(BaseAlgo):

    config: FastTD3Config
    env: FastTD3Env  # type: ignore[assignment]
    actor: Actor
    qnet: Critic

    def __init__(
        self, env: BaseTask, config: FastTD3Config, device: str, log_dir: str, multi_gpu_cfg: dict | None = None
    ):
        wrapped_env = FastTD3Env(env, config.actor_obs_keys, config.critic_obs_keys)
    ...
    def setup(self) -> None:
        logger.info("Setting up FastTD3")
        ...
        self.actor = Actor(
            ...
            init_scale=args.actor_init_scale,
    ...
    def _synchronize_model_parameters(self):
        """Synchronize actor and qnet parameters across all GPUs."""
    ...
    def _update_main(
        ...
        with self._maybe_amp():
            ...
            with torch.no_grad():
                ...
                qf1_next_target_dist, qf2_next_target_dist = qnet_target.projection(
                ...
                use_cdq = getattr(args, "use_cdq", True)
                if use_cdq:
                ...
                target_value_max = qf1_next_target_value.max()
                target_value_min = qf1_next_target_value.min()
                ...
                with self.logging_helper.record_learn_time():
                    ...
                    for i, data in enumerate(prepared_batches):
                        ...
                        current_metrics = {
                            ...
                            "alpha_loss": torch.tensor(0.0, device=self.device),
                            "alpha_value": torch.tensor(0.0, device=self.device),
    ...
    def export(self, onnx_file_path: str) -> None:
        ...
        For storing checkpoints to resume training, see `FastTD3Agent.save()`
        """
